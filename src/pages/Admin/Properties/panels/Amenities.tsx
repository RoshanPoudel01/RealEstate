...
const Amenities: FC<AmenitiesProps> = ({ setTabValue }) => {
  const { id } = useParams();
  const defaultValues = {
  ...
  const { data: amenities, isLoading } = useFetchAmenities(id!);
  ...
  const { mutateAsync: createAmenities, isPending: isCreatingAmenities } =
    useUpdateAmenities();
  ...
  return !!id && isLoading ? (
    <Loader />
  ) : !!id && !isLoading && !amenities ? (
    <NotFound />
  ) : (
    <Flex flexDir={"column"} gap={4}>
      <SimpleGrid columns={{ base: 2, sm: 3 }} gap={4} asChild>
        <form id="amenities-form" onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      <HStack mt={4}>
        <Button onClick={() => setTabValue("general")} variant="outline">
          Back
        </Button>
        <Button
          form="amenities-form"
          type="submit"
          loading={isCreatingAmenities}
          loadingText="Saving"
          colorScheme="primary"
        >
          Save & Next
